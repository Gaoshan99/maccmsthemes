    ...
    'User':{
        ...
        'Init':function(){
            ...
            if(MAC.Cookie.Get('user_id') !=undefined && MAC.Cookie.Get('user_id')!=''){
                ...
                MAC.User.Portrait = maccms.path+MAC.Cookie.Get('user_portrait');
                MAC.User.IsLogin = 1;
                ...
                var html = '';
                html+='<dl class="layui-nav-child"><dd><a href="'+url+'">修改信息</a></dd><dd><a href="'+url+'">用户中心</a></dd><dd><a href="javascript:;" onclick="MAC.User.Logout();" target="_self">安全退出</a></dd></dl>'

                $('.mac_user').append(html);
				$('.user_name').text(MAC.User.UserName);
				/*狼哥修改*/
				/*设置头像*/
				var suerimg= MAC.User.Portrait; 
				
				//alert(suerimg);
				 
				//$(".face").attr("src",""+suerimg+"");
        ...
        'Login':function(){
            var ac='ajax_login';
			var loginWidth=$(window).width();//获取屏幕宽度
			if(loginWidth>1024){
				//电脑端弹出层宽度
				var widths="400";
              }else{
			...
			function suertc(){
				MAC.Pop.Show(widths,380,'用户登录',maccms.path+'/index.php/user/'+ac,function($r){
			...
			var UserWidth=$(window).width();//获取屏幕宽度
            if(MAC.Cookie.Get('user_id') !=undefined && MAC.Cookie.Get('user_id')!=''){
                ac= 'ajax_info';
            }else{
				 suertc();
